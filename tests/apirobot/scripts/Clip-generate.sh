...
while [ -z "${OLD_CLIP}" ] && [ "$TIME_LIMIT" -gt 0 ];
do
    echo "Polling legacy clip. Seconds left ${TIME_LIMIT}";
    export OLD_CLIP=$(curl -X GET \
        "https://app.${API_DOMAIN}/v1/user/userClips?devices=${DEVICE_ID}&access_token=${API_TOKEN}" 2>/dev/null | \
        jq --raw-output ".response.items[] | select(.userDescription == \"${CLIP_ID}\")");
    sleep 1;
    TIME_LIMIT=$(($TIME_LIMIT-1));
done
curl -L -o /tmp/old_clip.mp4 $(echo ${OLD_CLIP} | jq --raw-output .playUrls.https | head -1)
